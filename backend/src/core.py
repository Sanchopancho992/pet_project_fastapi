...
from .constants import TARGET, M, N, Player
...
def init_board() -> list[list[int]]:
    return [[Player.NONE for _ in range(M)] for _ in range(N)]


def valid_move(board: list[list[int]], row: int | None, col: int | None) -> bool:
    ...
    if board[row][col] != Player.NONE:
        return False
    return row == N - 1 or board[row + 1][col] != Player.NONE
...
def detect_winner(board: list[list[int]]) -> int | None:
    ...
    for i in range(N):
        for j in range(M):
            if board[i][j] != Player.NONE and check_directions(i, j):
                return board[i][j]
    return None
